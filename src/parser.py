...
class ParserTemplate:
	...
	def __init__(self):
		...
		self.parser_scan.add_argument('-t', '--targets', nargs='+', action = "store", metavar = "TARGET", required = True, help = "targets for which the hash value will be calculated")
		self.parser_scan.add_argument('-c', '--calculate', nargs='+', action = "store", metavar = "HASH_FUNCTION_NAME", required = True, help = "hash functions which will be used for the hash value calculation")
		...
		self.parser_scan.add_argument('-r', '--recursive', action = "store_true", help = "allows recursive scan of directories")
		...
		self.parser_compare.add_argument('-fuzzy', metavar = 'FUZZY_HASH_FUNCTION_NAME', action = "store", help = "fuzzy hash function which will be used for the similarity comparsion")
		self.parser_compare.add_argument('-t', '--targets', nargs='+', action = "store", metavar = "TARGETS_PARAMETER", required = True, help = "targets for which the hash value will be calculated")
		self.parser_compare.add_argument('-dw', '--download-location', action = "store", metavar = "DOWNLOAD_LOCATION", default = getcwd(), help = "path to location where remote targets will be downloaded to. default: current working directory")
		self.parser_compare.add_argument('-l', '--limit', action = "store", default = 1, type = int, metavar = "LIMIT", help = "only show results >= limit. default: 1")
		self.parser_compare.add_argument('-r', '--recursive', action = "store_true", help = "allows recursive comparsion with the contents of directories")
		...
		comparsion_args = self.parser_compare.add_mutually_exclusive_group(required=True)
